    ...
    def occupied(self, policy):
        for socket in self.sockets:
            if socket.occupied(policy):
                #print('for policy {}, node occupied returnes 1'.format(policy))
                return 1
        #print('for policy {}, node occupied returnes 0'.format(policy))
        return 0

    def free(self, jobs):
        #print('in node, freeing :',end='')
        #for i in jobs:
            #print("Freeing ID:{} name:{}".format(i.id,i.app))
        self.exclusive = 0
        for socket in self.sockets:
            socket.free(jobs)
